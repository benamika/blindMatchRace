	...
	protected void onPostExecute(Map<Long, LatLng> sortedLatLngs) {
		if (sortedLatLngs != null && sortedLatLngs.size() > 1) {
			if (kmlVer == 1) {
				...
				/*
				 * 
				 *כאן עשינו תשינוי מיכה!
				 *(מתחת להערה הזאת)
				 * 
				 */
				
				
				//We've used SimpleDateFormat library for converting the Unix-time into TimeZone
				Date d = new Date(starttime*1000L);
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss"); 
				sdf.setTimeZone(TimeZone.getTimeZone("GMT-4"));
				String formattedDate = sdf.format(d);
				formattedDate+='Z';
				
				kmlBuilder.append("\t\t\t<when>" + formattedDate + "</when>\n\t\t</TimeStamp>\n");
